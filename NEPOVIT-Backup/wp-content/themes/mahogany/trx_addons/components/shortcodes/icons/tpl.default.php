...
$icon_empty = ($args['icons'][0]['icon'] == 'none') ? ' empty_icon' : '';;
...
?><div id="<?php echo esc_attr($args['id']); ?>" 
	class="sc_icons sc_icons_<?php
		...
		mahogany_show_layout($icon_empty);
	?>"<?php
		...
		?><div class="sc_icons_item<?php echo !empty($item['link']) ? ' sc_icons_item_linked' : ''; ?>"><?php
			if (!empty($item['image'])) {
				...
				if (!empty($item['image'])) {
					...
					?><div class="sc_icons_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr__('img', 'mahogany'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			} else {
