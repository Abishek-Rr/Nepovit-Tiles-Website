<div class="front_page_section front_page_section_testimonials<?php
			$mahogany_scheme = mahogany_get_theme_option('front_page_testimonials_scheme');
			if (!mahogany_is_inherit($mahogany_scheme)) echo ' scheme_'.esc_attr($mahogany_scheme);
			echo ' front_page_section_paddings_'.esc_attr(mahogany_get_theme_option('front_page_testimonials_paddings'));
		?>"<?php
		$mahogany_css = '';
		$mahogany_bg_image = mahogany_get_theme_option('front_page_testimonials_bg_image');
...
?>><?php
	// Add anchor
	$mahogany_anchor_icon = mahogany_get_theme_option('front_page_testimonials_anchor_icon');	
	$mahogany_anchor_text = mahogany_get_theme_option('front_page_testimonials_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_testimonials_inner<?php
			if (mahogany_get_theme_option('front_page_testimonials_fullheight'))
				echo ' mahogany-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$mahogany_css = '';
			$mahogany_bg_mask = mahogany_get_theme_option('front_page_testimonials_bg_mask');
			$mahogany_bg_color = mahogany_get_theme_option('front_page_testimonials_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$mahogany_caption = mahogany_get_theme_option('front_page_testimonials_caption');
			if (!empty($mahogany_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo !empty($mahogany_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses($mahogany_caption, 'mahogany_kses_content'); ?></h2><?php
			}
		
			// Description (text)
			$mahogany_description = mahogany_get_theme_option('front_page_testimonials_description');
			if (!empty($mahogany_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo !empty($mahogany_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses(wpautop($mahogany_description), 'mahogany_kses_content'); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_testimonials_output"><?php 
				if (is_active_sidebar('front_page_testimonials_widgets')) {
					...
					if (!mahogany_exists_trx_addons())
						...
						mahogany_customizer_need_widgets_message('front_page_testimonials_caption', 'ThemeREX Addons - Testimonials');
				}
			?></div>
		</div>
	</div>
