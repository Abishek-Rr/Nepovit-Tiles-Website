...
if (!function_exists('mahogany_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'mahogany_hovers_theme_setup3', 3 );
	function mahogany_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		mahogany_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'mahogany'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'mahogany') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'mahogany'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'mahogany'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'mahogany') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
...
if (!function_exists('mahogany_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'mahogany_hovers_theme_setup9', 9 );
	function mahogany_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'mahogany_hovers_frontend_scripts', 1010 );
		add_filter( 'mahogany_filter_localize_script','mahogany_hovers_localize_script' );
		add_filter( 'mahogany_filter_merge_scripts',	'mahogany_hovers_merge_scripts' );
		add_filter( 'mahogany_filter_merge_styles',	'mahogany_hovers_merge_styles' );
		add_filter( 'mahogany_filter_get_css', 		'mahogany_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'mahogany_hovers_frontend_scripts' ) ) {
	
	function mahogany_hovers_frontend_scripts() {
		if ( mahogany_is_on(mahogany_get_theme_option('debug_mode')) && mahogany_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'mahogany-hovers', mahogany_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( mahogany_is_on(mahogany_get_theme_option('debug_mode')) && mahogany_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'mahogany-hovers',  mahogany_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('mahogany_hovers_merge_scripts')) {
	
	function mahogany_hovers_merge_scripts($list) {
...
if (!function_exists('mahogany_hovers_merge_styles')) {
	
	function mahogany_hovers_merge_styles($list) {
...
if (!function_exists('mahogany_hovers_localize_script')) {
	
	function mahogany_hovers_localize_script($arr) {
		$arr['button_hover'] = mahogany_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('mahogany_hovers_add_icons') ) {
	function mahogany_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat']) && $product->is_purchasable() && $product->is_in_stock()) {
					mahogany_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'mahogany') : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'mahogany');
						else
							esc_html_e('Details', 'mahogany');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-eye-light"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'mahogany_hovers_get_css' ) ) {
	
	function mahogany_hovers_get_css($css, $colors, $fonts, $scheme='') {
